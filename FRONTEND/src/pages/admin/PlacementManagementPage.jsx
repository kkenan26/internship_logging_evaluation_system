...
const PlacementManagementPage = () => {
  ...
  return (
    <div>
      ...
      <button onClick={openCreate} style={{ background: '#333', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '4px', cursor: 'pointer', marginBottom: '20px' }}>+ New Placement</button>
      {msg && <div style={{ background: '#ffe0e0', padding: '10px', marginBottom: '16px' }}>{msg}</div>}
      <table border="1" cellPadding="8" style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr><th>Student</th><th>Company</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          {placements.map(p => (
            <tr key={p.id}>
              ...
              <td>{p.status}</td>
              <td><button onClick={() => openEdit(p)}>Edit</button> <button onClick={() => handleDelete(p.id)}>Delete</button></td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', padding: '24px', borderRadius: '8px', width: '500px' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" disabled={submitting}>{submitting ? "Saving..." : editPlacement ? "Update" : "Create"}</button>
